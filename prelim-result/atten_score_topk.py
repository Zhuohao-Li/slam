...
plt.savefig('attention_scores_heatmap_topk.png')

# Optionally, display the combined heatmap
plt.show()

# Function to display top 10 attention score indices and values
def show_top_attention_indices(avg_attention, tokens, top_n=10, output_file='top_attention_scores.txt'):
    ...
    with open(output_file, 'w') as f:
        for i, token in enumerate(tokens):
            ...
            top_indices = np.argsort(attention_scores)[-top_n:][::-1]  # Get indices of top 10 scores
            top_values = attention_scores[top_indices]  # Get top 10 scores
    ...
    # Sort tokens by frequency and then by average score for visualization
    sorted_tokens = sorted(avg_scores.keys(), key=lambda x: (-top_token_frequency[x], -avg_scores[x]))
    most_frequent_tokens = sorted_tokens[:10]  # Adjust to show more/less frequent tokens as needed
    top_11_to_20_tokens = sorted_tokens[10:20]

    # Find indices for the most frequent tokens
    token_to_index = {token: i for i, token in enumerate(tokens)}
    most_frequent_indices = [token_to_index[token] for token in most_frequent_tokens]
    top_11_to_20_indices = [token_to_index[token] for toekn in top_11_to_20_tokens]

    # Visualization for top 10 indices
    plt.figure(figsize=(12, 6))
    plt.bar(range(len(most_frequent_indices)), [avg_scores[token] for token in most_frequent_tokens], color='#1FD665')
    plt.xlabel('Token Indices (Ranked by Frequency)')
    plt.ylabel('Average Attention Scores')
    plt.title('Top 10 Most Frequent Tokens by Attention Scores')
    plt.xticks(range(len(most_frequent_indices)), most_frequent_indices, rotation=45, ha='right')
    plt.tight_layout()

    # Add token and frequency labels on top of each bar
    for i, (index, token) in enumerate(zip(most_frequent_indices, most_frequent_tokens)):
        plt.text(i, avg_scores[token], f'freq: {top_token_frequency[token]}', 
                ha='center', va='bottom', fontsize=8)
    # Save the visualization
    plt.savefig('top_10_most_frequent_tokens.png')
    plt.show()
    

    print(f"Top attention scores saved to {output_file}")

# Display top 10 attention indices for each token
show_top_attention_indices(avg_attention, tokens)
