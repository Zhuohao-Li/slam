...
def analyze_attention(input_text, model_name='gpt2', top_n=10, output_file='attention_analysis.txt'):
    ...
    # Dictionary to track frequency of tokens in top N scores
    top_token_frequency = defaultdict(int)
    top_token_scores = defaultdict(list)  # To keep track of scores for averaging

    with open(output_file, 'w') as f:
        for i, token in enumerate(tokens):
            attention_scores = avg_attention[i, :]
            top_indices = np.argsort(attention_scores)[-top_n:][::-1]  # Get indices of top N scores
            top_values = attention_scores[top_indices]  # Get top N scores

            f.write(f"Token: '{token}' (Position {i+1})\n")
            f.write(f"Top {top_n} attention scores:\n")
            for idx, value in zip(top_indices, top_values):
                f.write(f"  Index: {idx}, Token: '{tokens[idx]}', Score: {value:.4f}\n")
                top_token_frequency[tokens[idx]] += 1  # Increment frequency
                top_token_scores[tokens[idx]].append(value)  # Append score for averaging
            f.write("\n")

    # Compute average scores for tokens in top N
    avg_scores = {token: np.mean(scores) for token, scores in top_token_scores.items()}

    return tokens, avg_scores, top_token_frequency

def visualize_top_11_to_30(tokens, avg_scores, top_token_frequency, output_file='top_11_to_30_frequent_tokens.png'):
